...
function processConfigFile(shell, job) {
  ...
  return function (context, cb) {
    ...
    if (!file_configuration.hasOwnProperty('phases') || !file_configuration.phases.hasOwnProperty(context.phase)) {
      context.comment(`No any scripts for '${context.phase}' phase`);
      return cb(null, true);
    }

    var phase_config = file_configuration.phases[context.phase].join('\n');

    context.cmd(shellCommand(phase_config, shell, job), cb);
  };
}
